...
function stabilizeMobileI18nLayout(){
  ...
  nodes.forEach(el=>{
    if(!el.isConnected||el.closest('.hidden')||el.getClientRects().length===0)return;
    const key=el.dataset.i18n;
    const elText=i18n.el?.[key],enText=i18n.en?.[key];
    if(!elText||!enText)return;
    const cs=getComputedStyle(el);
    if(!['block','flex','grid','list-item','table-cell'].includes(cs.display))return;
    const width=el.getBoundingClientRect().width;
    if(!width)return;
    let maxHeight=0;
    [elText,enText].forEach(value=>{
      const clone=el.cloneNode(false);
      clone.removeAttribute('id');
      clone.removeAttribute('data-i18n');
      clone.classList.remove('i18n-stable');
    ...
    if(maxHeight>0){
      el.style.setProperty('--i18n-stable-height',`${Math.ceil(maxHeight)}px`);
      el.classList.add('i18n-stable');
    }
  });
}

function setLang(lang,remember=false){state.uiLang=lang;document.documentElement.lang=lang;$$('[data-lang]').forEach(b=>b.classList.toggle('active',b.dataset.lang===lang));$$('[data-i18n]').forEach(el=>{const v=i18n[lang]?.[el.dataset.i18n];if(v)el.textContent=v});if(remember)rememberUiLang(lang);window.dispatchEvent(new CustomEvent('kaptiono:languagechange',{detail:{lang}}));if(!state.file&&lang==='en')$('#languageSelect').value='english';syncLanguagePicker?.();if(state.exporting)setExportUi(state.exportStage,state.exportPct);updateModelHint?.();updateCompatibilityNote?.();updateProcessingModeLabel?.();updateExportButtons?.();refreshLocalModelDownloadModal?.();stabilizeMobileI18nLayout();queueMicrotask(()=>{try{refreshCloudAvailability()}catch{}});}
...
const CLOUD_QUOTA_STATUS_URL=new URL('quota',CLOUD_TRANSCRIBE_URL).href;
const CLOUD_QUOTA_STATUS_TTL=20000;
const modelLabels={
...
function formatCloudQuotaDuration(seconds){
  const total=Math.max(0,Math.floor(Number(seconds)||0));
  const mins=Math.floor(total/60);
  const hours=Math.floor(mins/60);
  const rest=mins%60;
  if(isGreekUI()){
    if(hours&&rest)return `${hours}ω ${rest}λ`;
    if(hours)return `${hours}ω`;
    return `${Math.max(0,mins)}λ`;
  }
  if(hours&&rest)return `${hours}h ${rest}m`;
  if(hours)return `${hours}h`;
  return `${Math.max(0,mins)}m`;
}
function normalizeCloudQuota(payload){
  const q=payload?.quota||payload?.kaptiono?.quota||null;
  if(!q||q.configured===false)return null;
  const remainingSeconds=Number(q.remaining_seconds);
  const remainingNeurons=Number(q.remaining_neurons);
  if(!Number.isFinite(remainingSeconds)&&!Number.isFinite(remainingNeurons))return null;
  return {
    configured:true,
    remainingSeconds:Math.max(0,Number.isFinite(remainingSeconds)?remainingSeconds:0),
    remainingNeurons:Math.max(0,Number.isFinite(remainingNeurons)?remainingNeurons:0),
    usedNeurons:Math.max(0,Number(q.used_neurons)||0),
    allocationNeurons:Math.max(0,Number(q.allocation_neurons)||10000),
    resetAt:q.reset_at||new Date(nextCloudQuotaResetUtc()).toISOString(),
    updatedAt:Date.now()
  };
}
function applyCloudQuotaPayload(payload){
  const quota=normalizeCloudQuota(payload);
  if(!quota)return false;
  state.cloudQuota=quota;
  state.cloudQuotaLastFetch=Date.now();
  const resetMs=Date.parse(quota.resetAt)||nextCloudQuotaResetUtc();
  if(quota.remainingSeconds<=0){
    try{localStorage.setItem(CLOUD_QUOTA_STORAGE_KEY,String(resetMs))}catch{}
  }else{
    try{localStorage.removeItem(CLOUD_QUOTA_STORAGE_KEY)}catch{}
  }
  refreshCloudAvailability();
  renderCloudQuotaStatus();
  return true;
}
function renderCloudQuotaStatus(){
  const el=$('#cloudQuotaStatus');
  if(!el)return;
  const cloud=isCloudModelSelected();
  el.classList.toggle('hidden',!cloud);
  if(!cloud)return;
  el.classList.remove('warning','unavailable');
  const label=el.querySelector('span');
  if(state.cloudQuotaLoading&&!state.cloudQuota){
    label.textContent=isGreekUI()?'Έλεγχος διαθέσιμου Cloud…':'Checking Cloud availability…';
    return;
  }
  const q=state.cloudQuota;
  if(!q){
    label.textContent=isGreekUI()?'Cloud High Accuracy':'Cloud High Accuracy';
    return;
  }
  const reset=cloudResetTimeLabel(Date.parse(q.resetAt)||nextCloudQuotaResetUtc());
  const remaining=formatCloudQuotaDuration(q.remainingSeconds);
  const videoSeconds=Number(video?.duration)||0;
  if(q.remainingSeconds<=0){
    el.classList.add('unavailable');
    label.textContent=isGreekUI()?`Cloud μη διαθέσιμο σήμερα · ανανέωση ${reset}`:`Cloud unavailable today · resets ${reset}`;
    return;
  }
  if(videoSeconds>0&&q.remainingSeconds+1<videoSeconds){
    el.classList.add('warning');
    label.textContent=isGreekUI()?`Cloud: ${remaining} διαθέσιμα · video ${formatCloudQuotaDuration(videoSeconds)}`:`Cloud: ${remaining} left · video ${formatCloudQuotaDuration(videoSeconds)}`;
    return;
  }
  label.textContent=isGreekUI()?`Cloud σήμερα: ~${remaining} διαθέσιμα · ανανέωση ${reset}`:`Cloud today: ~${remaining} left · resets ${reset}`;
}
async function fetchCloudQuotaStatus({force=false}={}){
  if(!force&&state.cloudQuota&&Date.now()-state.cloudQuotaLastFetch<CLOUD_QUOTA_STATUS_TTL){
    renderCloudQuotaStatus();
    return state.cloudQuota;
  }
  if(state.cloudQuotaLoading)return state.cloudQuota;
  state.cloudQuotaLoading=true;
  renderCloudQuotaStatus();
  try{
    const response=await fetch(CLOUD_QUOTA_STATUS_URL,{method:'GET',cache:'no-store',headers:{'Accept':'application/json'}});
    if(!response.ok)return state.cloudQuota;
    const payload=await response.json();
    applyCloudQuotaPayload(payload);
    return state.cloudQuota;
  }catch(error){
    console.warn('Cloud quota status unavailable',error);
    return state.cloudQuota;
  }finally{
    state.cloudQuotaLoading=false;
    renderCloudQuotaStatus();
  }
}
async function ensureCloudQuotaForVideo(durationSeconds){
  const quota=await fetchCloudQuotaStatus({force:true});
  if(!quota)return true;
  if(quota.remainingSeconds<=0){
    markCloudQuotaExhausted(Date.parse(quota.resetAt)||nextCloudQuotaResetUtc());
    alert(isGreekUI()?'Το Cloud High Accuracy δεν είναι διαθέσιμο για σήμερα. Επίλεξε Local AI ή δοκίμασε ξανά μετά την ημερήσια ανανέωση.':'Cloud High Accuracy is unavailable for today. Choose Local AI or try again after the daily reset.');
    return false;
  }
  const needed=Math.max(0,Number(durationSeconds)||0);
  if(needed&&quota.remainingSeconds+1<needed){
    renderCloudQuotaStatus();
    alert(isGreekUI()
      ?`Δεν υπάρχει αρκετός Cloud χρόνος για αυτό το video. Χρειάζονται περίπου ${formatCloudQuotaDuration(needed)}, ενώ απομένουν περίπου ${formatCloudQuotaDuration(quota.remainingSeconds)} σήμερα.`
      :`There is not enough Cloud time for this video. It needs about ${formatCloudQuotaDuration(needed)}, while about ${formatCloudQuotaDuration(quota.remainingSeconds)} remains today.`);
    return false;
  }
  return true;
}
...
function scheduleCloudQuotaRefresh(until){
  ...
  state.cloudQuotaTimer=setTimeout(()=>{state.cloudQuotaTimer=null;state.cloudQuota=null;state.cloudQuotaLastFetch=0;refreshCloudAvailability();if(isCloudModelSelected())fetchCloudQuotaStatus({force:true})},delay);
}
...
function markCloudQuotaExhausted(serverUntil=0){
  const until=Number(serverUntil)||nextCloudQuotaResetUtc();
...
function isCloudInsufficientResponse(result){
...
function buildLanguagePicker(){
  ...
  Array.from(select.options).forEach(nativeOption=>{
    ...
    option.dataset.languageValue=nativeOption.value;
    const label=document.createElement('span');
    label.className='language-option-name';
...
function syncLanguagePicker(){
  ...
  const buttonLabel=button.querySelector('.language-picker-label');
  if(buttonLabel)buttonLabel.textContent=selected?.textContent?.trim()||'';
  const nativeByValue=new Map(Array.from(select.options).map(option=>[option.value,option]));
  menu.querySelectorAll('.language-picker-option').forEach(option=>{
    const nativeOption=nativeByValue.get(option.dataset.languageValue);
    const label=option.querySelector('.language-option-name');
    if(label&&nativeOption)label.textContent=nativeOption.textContent.trim();
    const active=option.dataset.languageValue===select.value;
    option.classList.toggle('active',active);
    option.setAttribute('aria-selected',active?'true':'false');
  });
}
...
$('#languageSelect')?.addEventListener('change',syncLanguagePicker);
buildLanguagePicker();

document.addEventListener('click',event=>{
  const modelPicker=$('#modelPicker');
  const languagePicker=$('#languagePicker');
  if(modelPicker&&!modelPicker.contains(event.target))setModelPickerOpen(false);
  if(languagePicker&&!languagePicker.contains(event.target))setLanguagePickerOpen(false);
});
...
async function generateCaptions(){
  ...
  if(cloud){
    const quotaOk=await ensureCloudQuotaForVideo(video.duration);
    if(!quotaOk)return;
  }
...
async function transcribeCloudAudio(audio,audioDuration){
  ...
  try{
    const headers={'Content-Type':'audio/wav','X-Kaptiono-Audio-Duration':String(Math.max(0,Number(audioDuration)||0))};
    ...
    if(isCloudInsufficientResponse(result)){
      applyCloudQuotaPayload(result);
      throw new Error('CLOUD_INSUFFICIENT_FOR_VIDEO');
    }
    if(!response.ok){
      if(isCloudQuotaLimitResponse(response,result,raw)){
        const serverReset=Date.parse(result?.quota?.reset_at||result?.kaptiono?.quota?.reset_at||'')||0;
        markCloudQuotaExhausted(serverReset);
        throw new Error('CLOUD_DAILY_QUOTA_EXHAUSTED');
      }
...
function socialExportAvailable(){
  return typeof window.VideoEncoder!=='undefined'&&typeof window.VideoDecoder!=='undefined';
}
function selectedVideoExportFormat(){
  if(state.exportMode==='social'){
    return socialExportAvailable()?{mime:'video/mp4',ext:'mp4',label:'MP4',pipeline:'social'}:null;
  }
  const legacy=preferredVideoRecorderFormat();
  return legacy?{...legacy,pipeline:'legacy'}:null;
}
function ensureValidExportMode(){
  if(state.exportMode==='social'&&!socialExportAvailable()&&preferredVideoRecorderFormat()){
    state.exportMode='fast';
    try{localStorage.setItem('kaptiono-export-mode','fast')}catch{}
  }
}
function setExportMode(mode,{persist=true}={}){
  if(state.exporting)return;
  if(mode!=='social'&&mode!=='fast')return;
  if(mode==='social'&&!socialExportAvailable())return;
  if(mode==='fast'&&!preferredVideoRecorderFormat())return;
  state.exportMode=mode;
  if(persist){try{localStorage.setItem('kaptiono-export-mode',mode)}catch{}}
  updateExportModeUi();
  updateExportButtons();
  trackEvent('export_mode_selected',{mode});
}
function updateExportModeUi(){
  ensureValidExportMode();
  const socialAvailable=socialExportAvailable();
  const fastAvailable=!!preferredVideoRecorderFormat();
  $$('[data-export-mode]').forEach(btn=>{
    const mode=btn.dataset.exportMode;
    const unavailable=(mode==='social'&&!socialAvailable)||(mode==='fast'&&!fastAvailable);
    btn.classList.toggle('active',mode===state.exportMode);
    btn.classList.toggle('unavailable',unavailable);
    btn.disabled=unavailable||state.exporting;
    btn.setAttribute('aria-pressed',String(mode===state.exportMode));
    if(unavailable)btn.title=isGreekUI()?'Δεν υποστηρίζεται από αυτόν τον browser.':'Not supported by this browser.';
    else btn.removeAttribute('title');
  });
}
function updateExportButtons(){
  ensureValidExportMode();
  const ok=state.captions.length>0,format=selectedVideoExportFormat(),videoOk=ok&&!!format&&!state.exporting;
  $('#srtBtn').disabled=!ok;$('#txtBtn').disabled=!ok;
  $('#quickVideoExportBtn').disabled=!videoOk;$('#videoDownloadBtn').disabled=!videoOk;
  updateExportModeUi();
  const social=state.exportMode==='social';
  const unsupported=isGreekUI()?'Η επιλεγμένη λειτουργία export δεν υποστηρίζεται σε αυτόν τον browser.':'The selected export mode is not supported in this browser.';
  const label=format?(social?'MP4 · H.264 + AAC · Social Compatible':`${format.label} · Fast Export`):unsupported;
  $('#videoExportFormat').textContent=label;
  $('#quickExportHint').textContent=format?(social?(isGreekUI()?'Social Compatible · Recommended · καλύτερο για TikTok, Reels και Shorts.':'Social Compatible · Recommended · best for TikTok, Reels and Shorts.'):(isGreekUI()?'Fast Export · πιο γρήγορη δημιουργία video.':'Fast Export · quicker video creation.')):unsupported;
}
$$('[data-export-mode]').forEach(btn=>btn.addEventListener('click',()=>setExportMode(btn.dataset.exportMode)));
...
function chooseSocialFrameRate(rawFps){
  const fps=Number(rawFps)||30;
  return fps>=45?60:30;
}
function socialOutputSize(displayWidth,displayHeight){
  let w=Math.max(2,Number(displayWidth)||1080),h=Math.max(2,Number(displayHeight)||1920);
  const portrait=h>w;
...
function socialVideoBitrate(width,height,fps){
  const pixels=Math.max(1,width*height);
  const base1080=8_000_000;
  const resolutionFactor=Math.max(.45,Math.min(1.35,pixels/(1920*1080)));
  const fpsFactor=fps>=50?1.55:1;
...
async function renderSocialMp4(){
  ...
  try{
    ...
    if(!(await videoTrack.canDecode()))throw new Error('VIDEO_CODEC_NOT_DECODABLE');

    const [displayWidth,displayHeight,stats]=await Promise.all([
      videoTrack.getDisplayWidth(),
      videoTrack.getDisplayHeight(),
      videoTrack.computePacketStats(240).catch(()=>null)
    ]);
    const sourceFps=stats?.averagePacketRate||30;
    const fps=chooseSocialFrameRate(sourceFps);
    const {width,height}=socialOutputSize(displayWidth,displayHeight);
    const bitrate=socialVideoBitrate(width,height,fps);
    ...
    const conversion=await Conversion.init({
      ...
      video:{
        ...
        process:(sample)=>{
          if(firstTimestamp===null)firstTimestamp=Number(sample.timestamp)||0;
          ctx.fillStyle='#000';
          ctx.fillRect(0,0,width,height);
          if(typeof sample.drawWithFit==='function')sample.drawWithFit(ctx,{fit:'fill'});
          else sample.draw(ctx,0,0,width,height);
          const t=Math.max(0,(Number(sample.timestamp)||0)-firstTimestamp);
          drawCanvasCaption(ctx,width,height,t);
          return canvas;
        }
      },
      audio:{
    ...
    if(!conversion.isValid){
      const reasons=(conversion.discardedTracks||[]).map(x=>x.reason).filter(Boolean).join(',');
      throw new Error(`SOCIAL_EXPORT_UNSUPPORTED${reasons?':'+reasons:''}`);
    }
...
async function renderLegacyVideo(){
  const format=preferredVideoRecorderFormat();
  if(!format)throw new Error('LEGACY_EXPORT_UNSUPPORTED');
  ...
  try{
    ...
    const displayW=src.videoWidth||1080,displayH=src.videoHeight||1920;
    const {width:w,height:h}=socialOutputSize(displayW,displayH);
    ...
    const node=ac.createMediaElementSource(src),dest=ac.createMediaStreamDestination();
    node.connect(dest);dest.stream.getAudioTracks().forEach(t=>stream.addTrack(t));
    ...
    const stopped=new Promise((res,rej)=>{rec.onstop=res;rec.onerror=e=>rej(e.error||new Error('MediaRecorder error'));});
    rec.start(1000);setExportUi('render',0);
    let lastPct=-1;
    const draw=()=>{
      try{ctx.drawImage(src,0,0,w,h);drawCanvasCaption(ctx,w,h,src.currentTime)}catch{}
      const pct=Math.min(100,src.duration?src.currentTime/src.duration*100:0),rounded=Math.round(pct);
      if(rounded!==lastPct){lastPct=rounded;setExportUi('render',pct)}
...
async function exportVideo(){
  ...
  ensureValidExportMode();
  const format=selectedVideoExportFormat();
  if(!format){
    alert(isGreekUI()?'Η επιλεγμένη λειτουργία video export δεν υποστηρίζεται από αυτόν τον browser. Δοκίμασε την άλλη επιλογή ή κατέβασε SRT/TXT.':'The selected video export mode is not supported by this browser. Try the other option or download SRT/TXT.');
    return;
  }

  trackEvent('video_export_started',{caption_count:state.captions.length,format:format.ext,pipeline:format.pipeline,export_mode:state.exportMode});
  ...
  try{
    ...
    const suggestion=state.exportMode==='social'?(isGreekUI()?' Δοκίμασε Fast Export αν χρειάζεσαι άμεσα το αρχείο.':' Try Fast Export if you need the file immediately.'):(isGreekUI()?' Δοκίμασε Social Compatible για πιο ελεγχόμενο MP4.':' Try Social Compatible for a more controlled MP4.');
    alert((isGreekUI()?'Το video export απέτυχε: ':'Video export failed: ')+(e?.message||String(e))+suggestion);
  }finally{
    state.exporting=false;updateExportButtons();
    if(state.exportStage==='done')setTimeout(()=>resetExportUi(),1800);
    else if(state.exportStage==='error')setTimeout(()=>resetExportUi(),5000);
    else setTimeout(()=>resetExportUi(),1200);
  }
}
...
const INSTALL_FLAG='kaptiono-installed';
const INSTALL_DISMISS_KEY='kaptiono-install-dismissed-at';
const INSTALL_DISMISS_COOLDOWN=1000*60*60*24*14;
let deferredInstallPrompt=null;
let swRegistration=null;
let waitingWorker=null;
let waitingWorkerVersion='';
let reloadingForUpdate=false;

function markInstalled(){
  try{localStorage.setItem(INSTALL_FLAG,'1');localStorage.removeItem(INSTALL_DISMISS_KEY)}catch{}
}
function installationPromptOnCooldown(){
  try{
    const dismissedAt=Number(localStorage.getItem(INSTALL_DISMISS_KEY)||0);
    return dismissedAt>0&&Date.now()-dismissedAt<INSTALL_DISMISS_COOLDOWN;
  }catch{return false}
}
...
async function isAppAlreadyInstalled(){
  ...
  try{if(localStorage.getItem(INSTALL_FLAG)==='1')return true}catch{}
  if(typeof navigator.getInstalledRelatedApps==='function'){
    try{
      const apps=await navigator.getInstalledRelatedApps();
      if(Array.isArray(apps)&&apps.some(app=>app?.platform==='webapp')){markInstalled();return true}
    }catch{}
  }
  return false;
}
async function showInstallBanner(){
  if(await isAppAlreadyInstalled()||installationPromptOnCooldown())return;
  // Chromium exposes beforeinstallprompt only when the PWA is actually installable.
  // iOS has no browser API that can reliably inspect a Home Screen installation,
  // so the persistent confirmation/cooldown prevents repeated prompts there.
  if(isIOS||deferredInstallPrompt)$('#installBanner')?.classList.remove('hidden');
}
...
window.addEventListener('beforeinstallprompt',async e=>{
  ...
  if(!(await isAppAlreadyInstalled()))setTimeout(showInstallBanner,350);
});
...
$('#installDismiss')?.addEventListener('click',()=>{deferInstallPrompt();hideInstallBanner()});
$('#installAction')?.addEventListener('click',async()=>{
  if(await isAppAlreadyInstalled()){hideInstallBanner();return}
...
$('#iosInstallClose')?.addEventListener('click',()=>{deferInstallPrompt();closeIosInstallModal();hideInstallBanner()});
$('#iosInstallDone')?.addEventListener('click',()=>{
  // The user explicitly confirms that the Add to Home Screen steps are complete.
  markInstalled();
  closeIosInstallModal();
  hideInstallBanner();
});
$('#iosInstallModal')?.addEventListener('click',e=>{
  if(e.target.id==='iosInstallModal'){deferInstallPrompt();closeIosInstallModal();hideInstallBanner()}
});

if(isIOS&&!isStandalone())setTimeout(showInstallBanner,900);

function isCriticalWorkActive(){
  const progress=$('#progressBox');
  return !!state.file||state.exporting||!!(progress&&!progress.classList.contains('hidden'));
}
function updateWasDeferred(version){
  try{return sessionStorage.getItem('kaptiono-update-later-version')===version}catch{return false}
}
function showUpdateModal(version=''){
  if(!waitingWorker||!navigator.serviceWorker.controller||isCriticalWorkActive())return;
  const resolvedVersion=version||waitingWorkerVersion||'new';
  if(updateWasDeferred(resolvedVersion))return;
  waitingWorkerVersion=resolvedVersion;
  const label=$('#updateAvailableVersion');
  if(label)label.textContent=resolvedVersion==='new'?(state.uiLang==='el'?'Νέα έκδοση':'New version'):`v${resolvedVersion}`;
  $('#updateModal')?.classList.remove('hidden');
}
function hideUpdateModal(){$('#updateModal')?.classList.add('hidden')}
function deferCurrentUpdate(){
  const version=waitingWorkerVersion||'new';
  try{sessionStorage.setItem('kaptiono-update-later-version',version)}catch{}
  hideUpdateModal();
}
function prepareWaitingWorker(worker,version=''){
  if(!worker||!navigator.serviceWorker.controller)return;
  waitingWorker=worker;
  if(version)waitingWorkerVersion=version;
  showUpdateModal(waitingWorkerVersion);
}
...
$('#updateLaterBtn')?.addEventListener('click',deferCurrentUpdate);
$('#updateModal')?.addEventListener('click',e=>{if(e.target.id==='updateModal')deferCurrentUpdate()});
...
async function checkForAppUpdate(){
  ...
  try{
    ...
    if(info?.version&&info.version!==APP_VERSION)waitingWorkerVersion=String(info.version);
  }catch{}
  try{await swRegistration.update()}catch{}
  if(swRegistration.waiting)prepareWaitingWorker(swRegistration.waiting,waitingWorkerVersion);
}
...
if('serviceWorker' in navigator){
  ...
  window.addEventListener('load',async()=>{
    try{
      ...
      if(swRegistration.waiting)prepareWaitingWorker(swRegistration.waiting);
      swRegistration.addEventListener('updatefound',()=>{
        const installing=swRegistration.installing;
        if(!installing)return;
        installing.addEventListener('statechange',()=>{
          if(installing.state==='installed'&&navigator.serviceWorker.controller){
            prepareWaitingWorker(swRegistration.waiting||installing,waitingWorkerVersion);
          }
        });
      });
